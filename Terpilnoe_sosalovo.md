...
- *** Рубрыка "Цікавыя хвакты" ад бінга: ***
- Когда DLL используется, она загружается в память только один раз, и операционная система использует механизм файлового отображения для того, чтобы разные процессы могли использовать одну и ту же загруженную копию1. Файловое отображение — это буфер в физической памяти, отдельные части которого программы могут отображать в своё адресное пространство1. Это позволяет экономить ресурсы системы, так как не требуется загружать библиотеку для каждого процесса отдельно.
- Исходная DLL загружается в область памяти, которая является разделяемым ресурсом между процессами1. Операционная система управляет доступом к этой памяти и обеспечивает, чтобы при завершении работы одного из процессов, использующих DLL, другие процессы продолжали работать без сбоев1.
- Счётчиков процессов, которые используют DLL, как таковых нет, но операционная система поддерживает счётчик ссылок на DLL. Каждый раз, когда процесс вызывает LoadLibrary для DLL, счётчик ссылок увеличивается, а когда процесс вызывает FreeLibrary, счётчик уменьшается2. DLL остаётся загруженной в память до тех пор, пока счётчик ссылок не станет равным нулю, что означает, что ни один процесс больше не использует эту DLL2.
...
- link.exe (при автоматическом создании), lib.exe (при создании ручками)
